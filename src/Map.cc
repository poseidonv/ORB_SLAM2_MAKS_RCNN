...
void Map::SaveKeyFrame( ofstream &f, KeyFrame* kf )
{
    ...
    int Gridsize1 = kf->mGrid.size();
    f.write((char*)&Gridsize1,sizeof(Gridsize1));
    for (int i = 0; i < Gridsize1; i++)
    {
        int Gridsize2 = kf->mGrid[i].size();
        f.write((char*)&Gridsize2,sizeof(Gridsize2));
        for (int j = 0; j < Gridsize2; j++)
        {
            int Gridsize3 = kf->mGrid[i][j].size();
            f.write((char*)&Gridsize3,sizeof(Gridsize3));
            for (int k = 0;k<Gridsize3;k++)
            {
                size_t GridValue = kf->mGrid[i][j][k];
                f.write((char*)&GridValue,sizeof(GridValue));
            }
        }  
    }
...
void Map::Load ( const string &filename, SystemSetting* mySystemSetting)
{
    ...
    for ( auto kf: mspKeyFrames )
        kf_by_id[kf->mnId] = kf;
    cout<<"kf_by_id size: "<<kf_by_id.size()<<endl;
    cout<<"kf_by_id: requery: "<<kf_by_id[19]->mnRelocQuery<<endl;
...
KeyFrame* Map::LoadKeyFrame( ifstream &f, SystemSetting* mySystemSetting )
{
    ...
    kf->SetPose(T);
    kf->ComputeBoW();
    std::vector< std::vector <std::vector<size_t> > > myGrid;
    int Gridsize1;
    f.read((char*)&Gridsize1,sizeof(Gridsize1));
    myGrid.resize(Gridsize1);
    for (int i = 0; i < Gridsize1; i++)
    {
        int Gridsize2;
        f.read((char*)&Gridsize2,sizeof(Gridsize2));
        myGrid[i].resize(Gridsize2);
        for (int j = 0; j < Gridsize2; j++)
        {
            int Gridsize3;
            f.read((char*)&Gridsize3,sizeof(Gridsize3));
            myGrid[i][j].resize(Gridsize3);
            for (int k = 0; k < Gridsize3; k++)
            {
                size_t GridValue;
                f.read((char*)&GridValue,sizeof(GridValue));
                //cout<<"GridValue: "<<GridValue<<endl;
                myGrid[i][j][k] = GridValue;
            }
            
        }
        
    }
    //cout<<"Build myGrid finish!"<<endl;
    kf->mGrid = myGrid;
