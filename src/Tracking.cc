...
bool Tracking::IsNewSegImgArrived()
{
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    clock_t start = clock();
    while(!IsNewSegImgArrived())
    {
        usleep(1);
    }
    clock_t end = clock();
    // double ttrack= std::chrono::duration_cast<std::chrono::duration<double> >(t4 - t3).count();
    std::cout<<"wait segment use time : "<<(double)(end - start)/CLOCKS_PER_SEC<<"s"<<std::endl;

    mCurrentFrame.EraseMovingPoints(mImGray ,mvSegInfo[3]);
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                else
                {
                    ...
                    if(!bOK)
                    {
                        cout<<"motion -  TrackReferenceKeyFrame"<<endl;
                        bOK = TrackReferenceKeyFrame();
                        cout<<"bOK: "<<bOK<<endl;
                    }
                }
...
void Tracking::MonocularInitialization()
{
    ...
    else
    {
        ...
        // cv::Mat dst;
        // cv::drawMatches(img1, mInitialFrame.mvKeysUn, img2, mCurrentFrame.mvKeysUn, matches, dst);
...
void Tracking::CreateNewKeyFrame()
{
    ...
    mnLastKeyFrameId = mCurrentFrame.mnId;
    mpLastKeyFrame = pKF;
    //cout<<"Add new keyframe end!"<<endl;
}
